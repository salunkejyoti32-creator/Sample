...
function ViewAttendance() {
  ...
  const fetchAllAttendance = () => {
    ...
    fetch("http://localhost:8091/attendance/get-all-attendance-records/")
      .then((res) => {
        console.log('All attendance response status:', res.status);
        return res.json();
      })
      .then((data) => {
      ...
      .catch((err) => {
        ...
        alert(`Error fetching all attendance: ${err.message}`);
      });
  };
                ...
                {attendance.length === 0 ? (
                  ...
                  attendance.map((a, index) => (
                    <tr key={a.id || index} className="border-b hover:bg-gray-100">
                      ...
                      <td className="px-4 py-2">{a.subject ? a.subject.name : 'N/A'}</td>
                      <td className="px-4 py-2">{a.date || 'N/A'}</td>
                      <td className="px-4 py-2">{a.time || 'N/A'}</td>
                      <td className="px-4 py-2">{a.numberOfStudents || 0}</td>
                      <td className="px-4 py-2">
                        <button
