...
function AddSubject() {
  ...
  const handleDeleteSubject = (id) => {
    ...
    const endpoints = [
      `http://localhost:8091/subject/delete-subject/${id}/`,
      `http://localhost:8091/subject/delete-subject/${id}`,
      `http://localhost:8091/subject/delete/${id}/`,
      `http://localhost:8091/subject/${id}/`
    ];

    const tryDelete = async (endpointIndex = 0) => {
      if (endpointIndex >= endpoints.length) {
        alert("All delete endpoints failed. Subject may have attendance records.");
        return;
      }

      try {
        const res = await fetch(endpoints[endpointIndex], {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (res.ok) {
          ...
          fetchSubjects();
        } else if (res.status === 404 && endpointIndex < endpoints.length - 1) {
          console.log(`Endpoint ${endpoints[endpointIndex]} not found, trying next...`);
          tryDelete(endpointIndex + 1);
        } else {
          const errorText = await res.text();
          console.error('Delete error:', errorText);
          alert(`Failed to delete subject. Status: ${res.status}. May have attendance records.`);
        }
      } catch (err) {
